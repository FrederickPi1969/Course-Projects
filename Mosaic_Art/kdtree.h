...
class KDTree
{
  ...
  public:
    ...
    void deleteNode(KDTreeNode * current);
    KDTreeNode* copyNode(KDTreeNode * otherRoot);
    KDTreeNode* buildTree(vector<Point<Dim>>& v, int a, int b, int c);
    int partition(vector<Point<Dim>> & points, int start, int end, int dim_to_select);
    Point<Dim> & quickSelect(vector<Point<Dim>> & points, int start, int end, int dim_to_select, int median);
    Point<Dim> findNearest(KDTreeNode* current, const Point<Dim>& target, Point<Dim> & tempBest, int dim_to_check) const;
    /**
